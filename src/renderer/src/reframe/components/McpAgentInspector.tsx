...
export const McpAgentInspector: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-y-auto p-4 space-y-5 text-xs text-zinc-300 select-none">
      ...
      <div className="p-3.5 rounded-xl bg-zinc-900 border border-zinc-800 shadow-sm relative overflow-hidden space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2 pt-1 font-mono text-[11px]">
          <div className="p-2 rounded-xl bg-zinc-950 border border-zinc-800">
            <div className="text-[10px] text-zinc-400 uppercase tracking-wider">Port</div>
            <div className="font-semibold text-zinc-200">{status?.port || 3920}</div>
          </div>

          <div className="p-2 rounded-xl bg-zinc-950 border border-zinc-800">
            <div className="text-[10px] text-zinc-400 uppercase tracking-wider">Connected</div>
            <div className="font-semibold text-emerald-400">{status?.clientCount || 0} agents</div>
          </div>

          <div className="p-2 rounded-xl bg-zinc-950 border border-zinc-800">
            <div className="text-[10px] text-zinc-400 uppercase tracking-wider">Invocations</div>
        ...
        <div className="text-[11px] text-zinc-300 flex items-center justify-between pt-1">
          <span className="font-mono text-zinc-400 truncate max-w-[180px]">{sseEndpoint}</span>
          <button
        ...
        <div className="p-3 rounded-xl bg-zinc-900 border border-zinc-800 space-y-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-semibold text-zinc-100">Cursor / Antigravity / SSE</span>
            </div>
            <button
              onClick={() => copyToClipboard('cursor', cursorMcpConfig)}
              className="flex items-center gap-1 text-[10px] text-zinc-300 hover:text-white"
            >
              {copiedKey === 'cursor' ? (
                <Check className="w-3 h-3 text-emerald-400" />
              ) : (
        ...
        <div className="p-3 rounded-xl bg-zinc-900 border border-zinc-800 space-y-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-semibold text-zinc-100">Claude Desktop (Stdio)</span>
            </div>
            <button
          ...
          <p className="text-[11px] text-zinc-300 leading-relaxed">
            Paste into your{' '}
            <code className="text-zinc-200 font-mono">claude_desktop_config.json</code> to let
      ...
      <div className="space-y-2.5 flex-1 flex flex-col min-h-0">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] text-zinc-400 font-mono">{logs.length} calls</span>
        </div>

        <div className="flex-1 overflow-y-auto rounded-xl bg-zinc-950 border border-zinc-800 p-2.5 space-y-2 min-h-[140px] max-h-[260px] scrollbar-thin scrollbar-thumb-zinc-700">
          {logs.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center p-6 text-center text-zinc-400 space-y-1">
            ...
            logs.map((item) => (
              <div
                key={item.id}
                className="p-2 rounded-lg bg-zinc-900 border border-zinc-800 flex flex-col space-y-1 font-mono text-[11px]"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    {item.success ? (
