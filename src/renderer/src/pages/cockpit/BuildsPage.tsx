...
export function BuildsPage(): React.JSX.Element {
  ...
  return (
    <div className="mx-auto flex h-full max-w-[1200px] flex-col gap-4 p-5">
      {/* launcher */}
      <GlassCard
        icon={Hammer}
        ...
        subtitle={
          repo
            ? `${repo.scripts.length} npm script${repo.scripts.length === 1 ? '' : 's'} available — each one is a supervised child process`
      ...
      <div className="flex min-h-0 flex-1 gap-4">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <p className="mono mb-1.5 text-[10px] uppercase tracking-[0.14em] text-muted-foreground">
                ...
                {builds.map((build) => {
                  ...
                  return (
                    <div
                      key={build.id}
                      className={cn(
                        'group flex cursor-pointer items-center gap-3 px-3 py-2 transition-colors',
                        build.id === openBuildId ? 'bg-accent/40' : 'hover:bg-accent/20'
                      )}
                    ...
                    >
                      <span
                        ...
                        style={{
                          ...
                          boxShadow:
                            build.status === 'running' ? `0 0 6px ${tone.color}` : undefined
                        }}
                      />
                      <div className="min-w-0 flex-[2]">
                        <p className="mono truncate text-[12px] font-medium">
                      ...
                      <span className="mono shrink-0 text-[10.5px]" style={{ color: tone.color }}>
                        {tone.label}
                      </span>
                      {build.exitCode !== null && (
                      ...
                      <span className="mono w-[54px] shrink-0 text-right text-[10.5px] text-muted-foreground">
                        {elapsed(build)}
                      </span>
